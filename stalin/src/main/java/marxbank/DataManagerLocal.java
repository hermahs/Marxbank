...
public class DataManagerLocal {
    ...
    private static DataManagerLocal dataInstance = null;
     ...
     */
    public static DataManagerLocal manager() {
        if (dataInstance == null) {
            synchronized (DataManagerLocal.class) {
                if (dataInstance == null) {
                    dataInstance = new DataManagerLocal();
                }
            }
        }
    ...
    /**
     * Function to check if we are using the rest api or local api
     * @return true if localstoarge is enabled, false otherwise
     */
