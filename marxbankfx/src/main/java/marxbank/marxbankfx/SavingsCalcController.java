...
public class SavingsCalcController {
  ...
  @FXML
  private TextField monthlyAmount;
  @FXML
  private TextField lumpAmount;
  @FXML
  ...
  @FXML
  private Button findTotalAmount;
  ...
  /**
   * Setting the findTotalAmount button disabled if one of the textfields are empty or if the
   * interestRate text field is not on a decimal format.
   */
  @FXML
  private void findTotalAmountAble() {
    findTotalAmount.setDisable(monthlyAmount.getText().isEmpty() || lumpAmount.getText().isEmpty()
        || period.getText().isEmpty() || interestRate.getText().isEmpty()
        || !interestRateFieldValid());
  }


  /**
   * Calculates the total amount based on the values in text fields. If the value of type long is
   ...
   * 
   * @return true if a string containing only numbers has no comma in itself or if a string
   *         containing numbers on both side of one comma between them.
   */
