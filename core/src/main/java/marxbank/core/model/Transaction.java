...
/**
 * The purpose of the transaction class is to store information about a transaction between two
 * accounts. The information stored should never be changed, so it essentially functions as a
 * record, but it is also responsible for withdrawing and depositing the correct amount of balance
...
public class Transaction {
   ...
   */
  public Transaction(Long id, Account from, Account reciever, double amount, String date,
      boolean commit, boolean add) {
    validateId(id);
    this.id = id;
    validateFrom(from);
    this.from = from;
  ...
  /**
   * Initializes transaction object and runs the commitTransaction method if commit is true, with
  ...
  /**
   * Initializes transaction object and runs the commitTransaction method if commit is true, with
   * the current time as date and adds this transaction to from and recievers transaction list
   * 
   * @param id - id of transaction
   * @param from - Account that money is transfered from
   * @param reciever - Account that recieves money
  ...
  /**
   * Compact transaction constructor that generates id automatically, uses current time as date,
   * commits transaction and adds transaction to from and recievers transaction list, id is
   * generated with UUID
   * 
   * @param from - Account that money is transfered from
   * @param reciever - Account that recieves money
  ...
  /**
   * Getter for string representation of transaction date, which is useful for representing dates in
   * the UI.
   * 
   * @return dateString
   */
  ...
  /**
   * Getter for LocalDateTime representation of date, useful for comparing dates against eachother
   * and getting the current date by calling LocalDateTime.now().
   * 
   ...
   * 
   * @param from account to check
   * @return the account that was passed in
   ...
   * 
   * @param reciever account to check
   * @return account that was passed in
   * @throws IllegalArgumentException if account is null
  ...
  /**
   * checks if the transaction is between different users or not (used in backend).
   * 
  ...
  /**
   * Commits transaction by withdrawing from and depositing to the respective accounts.
   */
  ...
  /**
   * Converts a string representation of a date to LocalDateTime, given that it is represented
   * correctly according to the formatter of this class.
   * 
  ...
  public static LocalDateTime convertToDate(String dateString) {
    ...
    return LocalDateTime.of(Integer.parseInt(date[2]), Integer.parseInt(date[1]),
