...
/**
 * MarxAccount is a special type of account that has a maximum balance that cannot be exceeded when
 * depositing to the account. If a deposit to a MarxAccount results in a higher balance than the max
 * value, a new transaction will be created that transfers the excess amount to an account with the
 * lowest registered balance.
 */
public class MarxAccount extends Account {
  ...
  /**
   * Deposits the specified amount, and then checks if max balance has been exceeded. If that is the
   * case, creates a new transaction instance from this to them users account with the lowest
   * registered balance to get rid of the excess amount. If such an account cannot be found, the
  ...
  public void deposit(double amount) {
    ...
    if (this.getBalance() > MAX_BALANCE) {
      ...
      Account reciever =
          getUser().getAccounts().stream().filter(acc -> !(acc instanceof MarxAccount))
